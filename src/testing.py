...
# ):
#     """
#     Evaluate model on out-of-sample data with p-values for AUC and balanced accuracy (FDR corrected).
#     """
...
#     # --- AUC: DeLong CI + p-value vs 0.5 ---
#     auc, auc_var = delong_roc_variance(test_labels, y_pred_proba)
#     auc_std = np.sqrt(auc_var)
#     z = (auc - 0.5) / auc_std
#     p_auc = 1 - norm.cdf(z)
#     ci_auc = (auc - 1.96 * auc_std, auc + 1.96 * auc_std)
#     scores['auc_CI_lower'], scores['auc_CI_upper'], scores['auc_p'] = ci_auc[0], ci_auc[1], p_auc

#     # --- Balanced accuracy: approximate CI + binomial p-value vs 0.5 ---
#     sensitivity = tp / (tp + fn) if (tp + fn) > 0 else np.nan
#     specificity = tn / (tn + fp) if (tn + fp) > 0 else np.nan
#     var_bal = 0.25 * (
#         sensitivity * (1 - sensitivity) / (tp + fn)
#         + specificity * (1 - specificity) / (tn + fp)
#     )
#     se_bal = np.sqrt(var_bal)
#     ci_bal = (scores['balancedAcc'] - 1.96 * se_bal, scores['balancedAcc'] + 1.96 * se_bal)
#     p_bal = binomtest(
#         int(round(scores['balancedAcc'] * len(test_labels))),
#         len(test_labels),
#         p=0.5,
#         alternative='greater'
#     ).pvalue
#     scores['balancedAcc_CI_lower'], scores['balancedAcc_CI_upper'], scores['balancedAcc_p'] = ci_bal[0], ci_bal[1], p_bal

#     # --- Benjamini–Hochberg FDR correction (AUC + balancedAcc) ---
#     pvals = np.array([p_auc, p_bal])
#     labels = ['auc', 'balancedAcc']
#     _, pvals_fdr, _, _ = multipletests(pvals, alpha=0.05, method='fdr_bh')
#     scores['auc_p_fdr'], scores['balancedAcc_p_fdr'] = pvals_fdr

#     # --- Print summary ---
#     print("\nOut-of-sample performance:")
#     print(f"AUC = {auc:.3f} (95% CI: {ci_auc[0]:.3f}-{ci_auc[1]:.3f}), p={p_auc:.4f}, FDR={pvals_fdr[0]:.4f}")
#     print(f"BalancedAcc = {scores['balancedAcc']:.3f} (95% CI: {ci_bal[0]:.3f}-{ci_bal[1]:.3f}), p={p_bal:.4f}, FDR={pvals_fdr[1]:.4f}")

#     # --- SHAP feature importance (unchanged) ---
#     explainer = shap.TreeExplainer(best_model)
#     shap_values = explainer.shap_values(test_features)
#     if isinstance(shap_values, list):
#         shap_values = shap_values[1] if len(shap_values) > 1 else shap_values[0]
...
#     If top_n = 1, performs a bootstrap estimate of uncertainty for one model.
#     Each iteration resamples the test data (with replacement) to estimate 
#     variability of metrics.

#     Args:
#         res_dir: Directory containing model results and joblib files.
#         test_df: Test DataFrame with true labels.
#         target_col: Target variable column name.
#         group_name: Filter for model group in all_test_scores.csv.
#         top_n: Number of top models to evaluate (by AUC).
#         n_iterations: Number of bootstrap iterations per model.
#         desired_positive_rate: Optional target positive rate (float).
#                                If None, use the observed rate.
#         m: Number of samples per bootstrap (defaults to len(test_df)).
#         plot: Whether to plot metrics in test_oos.
#         target_var: Target variable name passed to test_oos.

#     Returns:
#         summary_df: DataFrame with mean and 95% CI (percentile) for each metric.
#         scores_df: Raw metrics from all resampling iterations.
#     """
...
#     # Compute mean and percentile-based 95% CI
#     summary_rows = []
...
#     plot=False,
#     target_var='responsive',
#     n_permutations=100           # number of label permutations per iteration
# ):
#     """
#     Bootstrapped evaluation of top models on a fixed test set, 
#     with permutation-based p-values for each metric.

#     If top_n = 1, performs bootstrap uncertainty + permutation significance.
...
#         plot: Whether to plot metrics in test_oos.
#         target_var: Target variable name passed to test_oos.
#         n_permutations: Number of label permutations per iteration.

#     Returns:
#         summary_df: DataFrame with mean, 95% CI, and permutation p-values.
#         scores_df: Raw metrics from all resampling iterations.
#     """
#     # Load test metrics to identify top models
#     df_scores = pd.read_csv(os.path.join(res_dir, "all_test_scores.csv"))
...
#     all_scores = []
#     perm_distributions = {}  # store permuted scores for each metric
...
#             # --- Permutation testing (fixed model, permuted labels) ---
#             perm_metrics = {metric: [] for metric in scores.keys()}
#             for _perm in range(n_permutations):
#                 perm_df = balanced_df.copy()
...
#             # accumulate permutation distributions
#             for metric, vals in perm_metrics.items():
#                 if metric not in perm_distributions:
#                     perm_distributions[metric] = []
#                 perm_distributions[metric].extend(vals)
...
#     pvals = summary_df['p (uncorrected)'].dropna().values
#     if len(pvals) > 0:
#         _, pvals_fdr, _, _ = multipletests(pvals, method='fdr_bh')
#         summary_df.loc[summary_df['p (uncorrected)'].notna(), 'p (FDR corrected)'] = pvals_fdr
#     else:
#         summary_df['p (FDR corrected)'] = np.nan
...
#     If top_n = 1, performs bootstrap-based uncertainty estimation for a single model.
#     Each iteration resamples the test data (with replacement) to assess metric variability.
...
#         n_iterations: Number of bootstrap iterations per model.
#         desired_positive_rate: Optional target positive rate (float). 
#                                If None, use observed rate.
#         m: Optional number of samples per bootstrap (defaults to len(test_df)).
#         plot: Whether to plot metrics in test_oos_logreg.
...
#     # Compute mean and percentile-based 95% CIs
#     summary_rows = []
...
#     """
#     Bootstrapped evaluation of top logistic regression models on a fixed test set,
#     with permutation-based p-values and FDR correction.
...
#     Returns:
#         summary_df: DataFrame with mean, 95% CI, permutation p-values, and significance.
#         scores_df: Raw metrics from all resampling iterations.
#     """
#     # Load test metrics to identify top models
#     df_scores = pd.read_csv(os.path.join(res_dir, "all_test_scores.csv"))
